...
export const clearDatabase = async () => {
  try {
    ...
    const deletePromises = snapshot.docs.map(docSnapshot => deleteDoc(doc(db, 'products', docSnapshot.id)));
...
export const seedProducts = async () => {
  try {
    ...
    toast.loading('Fetching sneaker data from RapidAPI...', { id: 'seedToast' });
    
    // Generic endpoint for searching Nike shoes on this API
    const response = await fetch('https://real-time-sneaker-prices.p.rapidapi.com/brand/nike?limit=20', {
      method: 'GET',
      headers: {
    ...
    const items = Array.isArray(apiData) ? apiData : (apiData.data || apiData.results || []);
    ...
    for (const item of items) {
      ...
      const product = {
        id: String(item.id || item._id || item.sku || `nike-${Math.random().toString(36).substr(2, 9)}`),
        ...
        description: item.description || item.story || 'Real-time sneaker data imported from RapidAPI.',
        image: item.image || item.thumbnail || item.imageUrl || 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?q=80&w=1200&auto=format&fit=crop',
        category: item.category || 'Lifestyle',
        colors: item.colors ? item.colors : (item.colorway ? [item.colorway] : ['#ffffff', '#000000']),
      };
      
      await setDoc(doc(productsRef, product.id), product);
    }
    
    toast.success('Sneaker Database seeded from API successfully!', { id: 'seedToast' });
  } catch (error) {
